...
interface ISprintNameInterface {
  id: string;
  title: string;
}
...
const Tasks = () => {
  ...
  const editNameHandle = () => {
    setShowInput(true);
    (document.querySelector('#inputChangeTitle') as HTMLElement).focus();
  };
  ...
  const changeTitleSubmit = (e: FormEvent) => {
    ...
    if (sprintName !== title || title !== '') {
      const obj: ISprintNameInterface = {
        id,
        title,
      };
  ...
  const onHandleChange = (e: FormEvent) => {
  ...
  useEffect(() => {
    if (filteredSprints.length !== 0) {
      ...
      setSprintName(filteredSprints[0].title);
    }
  ...
  const projectId = location.pathname.split('/')[2];
  const filterChange = (e: ChangeEvent) => {
